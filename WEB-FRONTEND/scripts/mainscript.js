...
var endpoint = "http://localhost:8080"; //TODO: passar para environment file (.env)
...
document.getElementById("name").addEventListener("input", function(event) { //Fazer tratamento correto do nome inserido.
    let inputValue = event.target.value;

    // Remove acentos e caracteres especiais
    inputValue = inputValue.normalize("NFD").replace(/[\u0300-\u036f]/g, "");

    // Remove caracteres especiais (permitindo apenas letras e números, sem espaços)
...
function fillInputs(playerName) { //Pega os inputs do formulario e preenche com os dados player.
    ...
    document.getElementById('inteligencia').value = playerData.inteligencia;
        ...
        function renderPlayers(players) {
            ...
            players.forEach(player => {
                ...
                playerCardHTML  +=`
                        ...
                        <div class="player-info" style="color: rgb(204, 204, 204);"><strong><img src="images/icons/name.jpg" class="player-state-image" alt="nome">Nome: </strong> ${player.name}</div>
                        ...
                        <div class="player-info" style="color: rgb(204, 204, 204);"><strong><img src="images/icons/int.jpg" class="player-state-image" alt="trauma">Inteligencia:</strong> ${player.inteligencia}</div>
                    </div>
        ...
        function deletePlayer(playerName) {
            ...
            fetch(`${endpoint}/delete_player?name=${playerName}`, { method: 'DELETE' })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
        ...
        function checkIfPlayerExists(name) {
            return fetch(`${endpoint}/check_player?name=` + name)
                .then(response => response.json())
    ...
    fetch(`${endpoint}/clear_players`, { method: 'POST' })
        .then(response => response.json())
        .then(data => {
            if (data.message) {
